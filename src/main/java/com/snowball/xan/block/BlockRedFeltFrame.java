...
public class BlockRedFeltFrame extends Block implements IHasModel {
	public static final PropertyDirection FACING = BlockDirectional.FACING;
	public static final PropertyEnum<EnumFaces> FACES = PropertyEnum.<EnumFaces>create("face", EnumFaces.class);
	public static final PropertyEnum<EnumRedColours> COLOUR = PropertyEnum.<EnumRedColours>create("colour", EnumRedColours.class);
	
	public BlockRedFeltFrame(String name) {
		super(Material.CLOTH);
		this.setUnlocalizedName(name);
		this.setRegistryName(name);
		this.setCreativeTab(Xan.XAN_TAB);
		
		this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(FACES, EnumFaces.BASE).withProperty(COLOUR, EnumRedColours.BLUE));
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) { 
		if (world.isRemote)
			...
			if (EnumRedColours.map.containsKey(item)) {
				EnumFaces faces = state.getValue(FACES);
				EnumRedColours colours = EnumRedColours.valueOf(item);
				
				IBlockState blockState = (IBlockState) ModBlocks.RED_FELT_FRAME;
	...
	public int getMetaFromState(IBlockState state) {
        ...
        i = i | ((EnumRedColours)state.getValue(COLOUR)).getId() << 2;
	...
	public IBlockState getStateFromMeta(int meta) {
		return this.getDefaultState().withProperty(FACES, EnumFaces.fromId(meta & 5)).withProperty(FACING, EnumFacing.getHorizontal((meta & 15) >> 2)).withProperty(COLOUR, EnumRedColours.fromId((meta & 15) >> 3));
	}
